#!/usr/bin/env python
"""A CLI for building an running RealEliza project locally."""
import click
import os
import subprocess
import sys
...
@click.option("--image-name", "-i", default="realeliza-next-web", help="Name of the Docker image.")
@click.option(
    "--rebuild", is_flag=True, help="Flag to indicate whether to rebuild the Docker image."
)
...
@click.option("--file", "-f", default="client/next-web/.env", help="Path to the .env file.")
@click.option(
    "--image-name", "-i", default="realeliza-next-web", help="The name of the Docker image to run."
)
