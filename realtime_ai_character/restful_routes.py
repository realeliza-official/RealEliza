...
@router.get("/status")
async def status():
    return {"status": "ok", "message": "RealEliza is running smoothly!"}
...
async def characters(user=Depends(get_current_user)):
    ...
    def get_image_url(character):
        if character.data and "avatar_filename" in character.data:
            ...
            return f"{gcs_path}/static/realeliza/{character.character_id}.jpg"
    ...
    return [
        {
            ...
            "audio_url": f"{gcs_path}/static/realeliza/{character.character_id}.mp3",
